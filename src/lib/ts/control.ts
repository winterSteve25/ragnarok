...
import {EDITOR_CONTEXT, FOCUS_ON_EDITOR} from "./stores";
...
export namespace KeyboardControl {
    ...
    const dirToFunction = {
        "ArrowLeft": KeybindHelper.moveCursorLeft,
        "ArrowRight": KeybindHelper.moveCursorRight,
        "ArrowUp": KeybindHelper.moveCursorUp,
        "ArrowDown": KeybindHelper.moveCursorDown,
    };
    const dirToOffset = {
        "ArrowLeft": [-1, 0],
        "ArrowRight": [1, 0],
    ...
    let currentlyFocusedItem: HTMLElement | null = null;

    export function onFocusIn(event: FocusEvent) {
        currentlyFocusedItem = event.target as HTMLElement;
        FOCUS_ON_EDITOR.set(currentlyFocusedItem && currentlyFocusedItem.classList.contains("Editor"));
    }
    ...
    export async function onKeyDown(event: KeyboardEvent) {
        ...
        if (!get(FOCUS_ON_EDITOR)) {
            
            if (!currentlyFocusedItem || !currentlyFocusedItem.classList.contains("navigable")) {
                return;
            }
            ...
            currentlyFocusedItem.dispatchEvent(new CustomEvent("navigate", {
                bubbles: true,
                composed: false, 
                cancelable: false, 
                detail: {
                    // @ts-ignore
                    direction: dirToOffset[event.key]
                }
            }));
    ...
    function insertMode(event: KeyboardEvent): boolean {
        ...
        if (event.key.startsWith("Arrow")) {
            // @ts-ignore
            const dir = dirToFunction[event.key];
